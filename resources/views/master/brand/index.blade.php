...
<form method="GET" action="{{ route('master.brand.index') }}" class="mb-3">
    <div class="input-group input-group-sm">
        <span class="input-group-text"><i class="fa-solid fa-search"></i></span>
        <input type="text" name="search" class="form-control" placeholder="Cari nama brand..." value="{{ request('search') }}">
        <button type="submit" class="btn btn-primary btn-sm px-3">Cari</button>
        @if(request('search'))
        <a href="{{ route('master.brand.index') }}" class="btn btn-outline-secondary btn-sm">
...
<div style="font-size:0.72rem;color:var(--text-dim);margin-bottom:8px">
    Total: <strong style="color:var(--text-primary)">{{ $data->total() }}</strong> brand
</div>
...
<div class="m-card mb-2">
    <div class="d-flex justify-content-between align-items-start mb-1">
        <div>
            <div style="font-weight:600;font-size:0.875rem;color:var(--text-primary)">{{ $item->nama_brand }}</div>
            @if($item->keterangan)
    ...
    <div class="d-flex gap-1 mt-2">
        <form method="POST" action="{{ route('master.brand.toggle', $item->id) }}" class="flex-fill">
            @csrf @method('PATCH')
        ...
        <button type="button" class="btn btn-sm btn-outline-secondary flex-fill py-1" style="font-size:0.72rem"
                ...
                data-id="{{ $item->id }}"
                data-nama="{{ $item->nama_brand }}"
                data-ket="{{ $item->keterangan }}">
            <i class="fa-solid fa-pen me-1"></i>Edit
        </button>
        <form method="POST" action="{{ route('master.brand.destroy', $item->id) }}"
              onsubmit="return confirm('Hapus brand \'{{ $item->nama_brand }}\'?')">
            @csrf @method('DELETE')
...
<div class="m-card text-center py-4" style="color:var(--text-dim)">
    ...
    <span style="font-size:0.85rem">Belum ada data brand.</span>
</div>
@endforelse
...
<div class="page-header">
    <div class="d-flex align-items-start justify-content-between flex-wrap gap-2">
        <div>
            <h2><i class="fa-solid fa-tag me-2" style="color:var(--accent)"></i>Master Brand</h2>
        ...
        <form method="GET" action="{{ route('master.brand.index') }}">
            <div class="row g-2 align-items-center">
                ...
                <div class="col-auto">
                    ...
                    @if(request('search'))
                        <a href="{{ route('master.brand.index') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
                    @endif
                </div>
            </div>
        </form>
...
<div class="card-dark">
    <div class="card-header d-flex align-items-center justify-content-between">
        <h6 class="card-title"><i class="fa-solid fa-table me-2"></i>Daftar Brand</h6>
            ...
            <table class="table table-dark-custom mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-weight:500">{{ $item->nama_brand }}</td>
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-1 justify-content-center">
                                <form method="POST" action="{{ route('master.brand.toggle', $item->id) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="btn btn-sm py-1 px-2 {{ $item->is_active ? 'btn-outline-warning' : 'btn-outline-success' }}" title="{{ $item->is_active ? 'Nonaktifkan' : 'Aktifkan' }}" style="font-size:0.7rem">
        ...
        <div class="modal-content modal-dark">
            <form method="POST" action="{{ route('master.brand.store') }}">
                ...
                <div class="modal-header modal-dark-header">
                    <h6 class="modal-title"><i class="fa-solid fa-plus-circle me-2" style="color:var(--accent)"></i>Tambah Brand</h6>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_brand" class="form-control form-control-sm" value="{{ old('nama_brand') }}" placeholder="Contoh: Alfamart" maxlength="100" required>
                    </div>
                    <div class="mb-1">
                        <label class="form-label">Keterangan</label>
                        <input type="text" name="keterangan" class="form-control form-control-sm" value="{{ old('keterangan') }}" placeholder="Opsional" maxlength="255">
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-dark">
            <form method="POST" id="formEdit" action="">
                @csrf @method('PUT')
                <div class="modal-header modal-dark-header">
                    <h6 class="modal-title"><i class="fa-solid fa-pen me-2" style="color:var(--accent)"></i>Edit Brand</h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Nama Brand <span class="text-danger">*</span></label>
                        <input type="text" name="nama_brand" id="edit_nama" class="form-control form-control-sm" maxlength="100" required>
                    </div>
                    <div class="mb-1">
                        <label class="form-label">Keterangan</label>
                        <input type="text" name="keterangan" id="edit_ket" class="form-control form-control-sm" maxlength="255">
                    </div>
                </div>
                <div class="modal-footer modal-dark-footer">
                    <button type="button" class="btn btn-outline-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
...
<script>
    document.getElementById('modalEdit').addEventListener('show.bs.modal', function (e) {
        ...
        document.getElementById('formEdit').action = '{{ url("master/brand") }}/' + btn.dataset.id;
        document.getElementById('edit_nama').value = btn.dataset.nama;
        document.getElementById('edit_ket').value  = btn.dataset.ket !== 'null' ? btn.dataset.ket : '';
    });
    @if($errors->any() && session('modal') === 'create')
